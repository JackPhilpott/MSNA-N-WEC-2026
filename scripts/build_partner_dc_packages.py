...
def non_idp_placemark(r):
    ...
    desc = (
        ...
        f"State / LGA / Ward: {r['adm1_name']} / {r['adm2_name']} / {r['adm3_name']}\n"
...
# ---------------------------------------------------------------------------
METADATA_COLUMNS = [
    "Partner", "Point Type", "State", "LGA", "Ward", "Cluster ID", "Survey ID",
...
def non_idp_metadata_row(partner, state_name, lga_name, r):
    ...
    return {
        ...
        "State": state_name, "LGA": lga_name, "Ward": r["adm3_name"],
...
README_DEFINITIONS = [
    ...
    ("Non-IDP household (reserve)", "A ranked backup household for a Non-IDP cluster, used strictly in rank order (Sequence column) whenever a primary household can't be reached or declines. Reserve list size always matches the primary target exactly for that cluster."),
...
for pcode, partners in partners_by_pcode.items():
    ...
    for partner in partners:
        ...
        wrote_a = write_kml(os.path.join(lga_dir, "non_idp_households_primary.kml"), "Non-IDP households (primary)", non_idp_primary)
        wrote_b = write_kml(os.path.join(lga_dir, "non_idp_households_reserve.kml"), "Non-IDP households (reserve)", non_idp_reserve)
        wrote_c = write_kml(os.path.join(lga_dir, "idp_clusters_primary.kml"), "IDP clusters (Tier 1 primary)", idp_primary)
        wrote_d = write_kml(os.path.join(lga_dir, "idp_clusters_tier2_backup.kml"), "IDP clusters (Tier 2 backup points)", idp_tier2_backup)
