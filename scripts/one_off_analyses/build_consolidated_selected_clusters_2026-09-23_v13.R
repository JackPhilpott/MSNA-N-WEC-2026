# ==============================================================================
# 2026-09-23: copied for the v12 -> v13 bump (FRAME_VERSION only), per the
# rebuild-not-rename rule - the v13 rds is built BEFORE any script is
# repointed to it (build_cluster_maps_production.R / build_lga_summary_
# maps.R's own references were already sed-swept to "v13" as part of this
# same bump's file sweep, so this must exist before either script next
# runs). This v13 bump is the R6 coverage-set change (INTERSOS-triggered
# review: Guzamala reinstated, 166 dead Nganzai points retired, Gwandu IDP
# reverted to excluded) - Jack confirmed the bump directly, and the
# Coordinator flagged this exact rebuild-not-rename trap by name (v6/v8/v9/
# v10 all had it: the sed sweep renamed the REFERENCE without rebuilding
# the file, silently staling or crashing the cluster + LGA summary maps).
#
# Logic unchanged from the v12 re-run (itself unchanged from 09-14, incl.
# its widened bare-name gpkg glob): union the Aug-6 design archive with
# every partner batch's new_clusters gpkg (geometry only, keyed by
# (uuid_hex, pop_type) - a staged file's cluster_id label is never
# trusted), then right-join v13 FULL's own cluster roster for the
# authoritative cluster_id + attributes. Site-level IDP draws have no
# uuid_hex and are skipped, as before - the maps script degrades those to
# has_hex=FALSE regardless.
# ==============================================================================
suppressMessages({
  library(dplyr)
  library(sf)
...
log_con <- file("output/gis/_consolidated_clusters_build_log_2026-09-23.txt", open = "wt")
