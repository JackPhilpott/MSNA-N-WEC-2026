...
# 2026-09-21: the v9/v10 references this line carried were both renamed by a
# version-bump sed sweep with no rebuild behind them (neither file ever
...
if (length(missing_uuid_hex) > 0) {
  cat(sprintf("hex_polygons: supplemented %d uuid_hex value(s) missing from hex_access, from selected_clusters_v13_current.rds.\n", length(missing_uuid_hex)))
}
...
stage2 <- read_csv(
  here::here(output_dir, "data", "data_collection", "NGA_MSNA_2026_stage2_sampling_frame_v13_WORKING.csv"),
