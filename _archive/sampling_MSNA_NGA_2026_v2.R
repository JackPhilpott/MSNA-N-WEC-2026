...
cache_rds <- function(file, rebuild = FALSE, fun){
  
  if (file.exists(file) && !rebuild)
    return(readRDS(file))
...
generate_start_points <- function(clusters) {
  
  pts <- purrr::map(
    clusters$geometry,
    ~ sf::st_sample(.x, size = 1, type = "random")
  )
  
  pts <- do.call(c, pts)
  
  sf::st_sf(
    sf::st_drop_geometry(clusters),
    geometry = pts,
    crs = sf::st_crs(clusters)
  )
}

host_start_points <- generate_start_points(host_clusters)
idp_start_points  <- generate_start_points(idp_clusters)
