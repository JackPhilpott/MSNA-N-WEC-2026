...
# ---- Pick good example clusters: well-populated, not below target ----

cluster_summary <-
  households %>%
  sf::st_drop_geometry() %>%
  group_by(cluster_id, pop_type, region, adm2_name, households_in_cluster, below_target_cluster) %>%
  summarise(n_primary = sum(status == "primary"), n_reserve = sum(status == "reserve"), .groups = "drop") %>%
  filter(!below_target_cluster, n_primary == 6, households_in_cluster >= 20, households_in_cluster <= 60) %>%
