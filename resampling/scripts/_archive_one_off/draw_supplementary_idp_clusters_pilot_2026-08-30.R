...
setwd(PROJECT_DIR)
STAGING_DIR <- file.path("resampling", "output", "resample_runs", "2026-08-30_intersos_fact_pilot")
...
shortfalls <- read_csv(
  "C:/Users/JACKPH~1/AppData/Local/Temp/claude/c--Users-JackPHILPOTT-ACTED-IMPACT-NGA---02--MSNA-4--Data-MSNA-N-WEC-2026/bbf40540-4359-44ed-a137-933cff5a411a/scratchpad/pilot_shortfalls_idp.csv",
...
tier1_hexes <- draw_idp_hexes(shortfalls, already_used_hex, seed = 91001)
log_msg("  Tier 1: %d hex(es) drawn", if (is.null(tier1_hexes)) 0 else nrow(tier1_hexes))
...
tier1_drawn_by_stratum <- if (is.null(tier1_hexes)) {
...
if (nrow(unresolved_strata) > 0) {
  ...
  tier2_hexes <- draw_idp_hexes(tier2_shortfalls, character(0), seed = 91002)
  log_msg("  Tier 2: %d hex(es) drawn (repeat draws allowed)", if (is.null(tier2_hexes)) 0 else nrow(tier2_hexes))
} else {
  tier2_hexes <- NULL
...
all_new_hexes <- dplyr::bind_rows(tier1_hexes, tier2_hexes)
...
if (nrow(all_new_hexes) == 0) stop("No IDP hexes were drawn in either tier - nothing to build sites from.")
...
idp_site_result <- select_stage2_idp_sites(
  ...
  mycrs = mycrs,
  cache_directory = file.path("resampling", "output", "cache", "pilot_idp_supplementary"),
  m = 6,
  rebuild = TRUE
)
